...
class EditTreatmentController extends GetxController {
  ...
  Future<TreatmentModel?> updateTreatment(bool isSuggest, int? id) async {
    final isValid = formKey.currentState!.validate();
    if (!isValid) {
      return null;
    }
    ...
    try {
      ...
      final treatment = await _treatmentServiceImpl.updateTreatment(
          treatment: treatmentModel, treatmentId: id);
      final listController = Get.find<TreatmentListController>();
      listController.getAllTreatments(
        sectorId,
        type,
  ...
  void suggestGlobalTreatment() async {
    try {
      ...
      inspect(treatment);
      Get.toNamed(Routes.createTreatmentFramer, arguments: treatment!.id);
    } catch (e) {
      inspect(e);
    } finally {
